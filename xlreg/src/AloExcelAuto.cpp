...
void CAloExcelAuto::AddinInstall(const char *fname)
{
	IDispatch *pXlAdds;
	IDispatch *pXlAdd;
	...
	AutoWrap(DISPATCH_METHOD, &result, m_instance.pXlBooks, L"Add", 0);
	m_instance.pXlBook = result.pdispVal;


	AutoWrap(DISPATCH_PROPERTYGET, &result, m_instance.pXlApp, L"AddIns", 0);
	pXlAdds = result.pdispVal;

	_bstr_t str = fname;
	VARIANT xla;
	xla.vt=VT_BSTR;
	xla.bstrVal = ::SysAllocString(str);
	AutoWrap(DISPATCH_METHOD, &result, pXlAdds, L"Add", 1, xla);
	pXlAdd = result.pdispVal;

	VARIANT bInst;
	bInst.vt=VT_BOOL;
	bInst.boolVal = true;
	AutoWrap(DISPATCH_PROPERTYPUT, NULL, pXlAdd, L"Installed", 1, bInst);

	pXlAdd->Release();
	pXlAdds->Release();
}

void CAloExcelAuto::AddinUninstall(const char *fname)
{
	IDispatch *pXlAdds;
	IDispatch *pXlAdd;
